...
import { setMapDetails } from "../../store/charts/chartsSlice";
...
const Map = () => {
  ...
  // const handleRegionMouseEnter = (regionName) => {
  ...
  // const handleRegionMouseLeave = (regionName) => {
  //   setIsHovered((prevState) => ({
  //     ...prevState,
  //     [regionName]: false,
  //   }));
  ...
  useEffect(() => {
    ...
    // Если хотя бы одно значение true, открываем диалоговое окно
  ...
  //   }
  //   renderChart()// <ChartModal className={`${isHovered.Batken ? "block" : "hidden"}`} />;
  // }, [isHovered]);
  ...
  return (
    <div className="relative w-full p-20">
      ...
      {isHovered.Chu && <ChartModal />}
      {isHovered.JalalAbad && <ChartModal />}
      {isHovered.Naryn && <ChartModal />}
      {isHovered.Osh && <ChartModal />}
      {isHovered.Talas && <ChartModal />}
      {isHovered.YsykKol && <ChartModal />} */}
    </div>
  );
};
